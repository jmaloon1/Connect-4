...
public class JackMaloonAI implements CFPlayer{
	...
	*/
	private class moveFinder{		
		...
		private HashMap<Integer, Integer> winnable_columns = new HashMap<>();   //HashMap of columns that are already considered winning
		...
		public void destroyWinningColumn() {
			
			System.out.println("wc " + winnable_columns);
			four_map = fourMap();
			boolean opp_can_win = false;
			
			for(int i: winnable_columns.keySet()) {    //looks at places on board where four in a row can be made and if two of same color on top of each other, indicates winning column
				
				int still_good_column = -1;
				
				for(int j=0;j<g.getNumRows(); j++) {

					if(j<g.getNumRows()-1 && (four_map[i][j]==AI_color || four_map[i][j]==2) && (four_map[i][j+1]== AI_color || four_map[i][j+1]==2)) {
			...
			if(winnable_columns.isEmpty()) {
				...
				while(winning_index.size()>1) {
					for(int key: winning_index) {
						if(winning_index.size()<2)
							break;
						...
						if(num<0) {
							...
							winning_index.remove((Object) key);
						}
						else {
				...
				if(!winning_column.contains(winning_index.get(0))) {
					winning_column.add(winning_index.get(0));
			...
			for(int i=0; i<g.getNumCols(); i++) {    //looks at places on board where four in a row can be made and if two of same color on top of each other, indicates winning column
				...
				for(int j=0;j<g.getNumRows(); j++) {
					...
					if((four_map[i][j]==AI_color || four_map[i][j]==2) && (j==0 || getState[i][j-1]!=0)) {
						...
						System.out.println(opp_color);
						System.out.println(four_map[i][j]);
					...
					if(j<g.getNumRows()-1 && four_map[i][j]==AI_color && (four_map[i][j+1]==AI_color || four_map[i][j+1]==2)) {
						...
						if(!opponent_can_win) {
							...
							System.out.print(winnable_columns);
							if(!winnable_columns.containsKey(i)) {
								System.out.println("wwwww " + i);
								winnable_columns.put(i, empty_squares);
								winning_index.add(i);
							}
						}
					}
			...
			for(int i=0; i<g.getNumCols(); i++) {
				for(int j=0; j<g.getNumRows()-1; j++) {
					...
					if(four_map[i][j]==AI_color && (four_map[i][j]==four_map[i][j+1] || four_map[i][j+1]==2)) {
						...
						if(!winning_column.contains(column) && !opp_can_win) {
							System.out.println("ttttttttttt " +column );
		...
		public void losingBoardCreator(int column, int row){
			...
			for(int c=0; c<g.getNumCols(); c++) {
				...
				for(int r=0; r<g.getNumRows(); r++) {
					...
					if((AI_blockable.contains(c) || AI_blockable.isEmpty()) && getState[c][r]==0 && (r==0||getState[c][r-1]!=0)) {
							...
							for(int j=0; j<g.getNumRows()-1; j++) {
								...
								if(four_map[i][j]==opp_color && (four_map[i][j]==four_map[i][j+1] || four_map[i][j+1]==2) 
								   && !loss_creator.contains(column)) {
									...
									System.out.println(AI_can_win);
									System.out.println(winning_index);
									if(!AI_can_win) {
										if(!winning_index.isEmpty()) {
											for(int index: winning_index) {
												if(empty_squares<winnable_columns.get(index)) {
	...
	public int columnQuality(CFGame g, moveFinder m, boolean print, ArrayList<Integer> good_sim_columns, ArrayList<Integer> bad_sim_columns) {			//plays the next move with sound logic
		...
		if(index!=-1)      //Looks to see if there is a column that can be played that guarantees win
			...
			return max_element_array(quality_array, good_sim_columns, bad_sim_columns, m.illegal_moves, m.losing_moves, m.unwise_moves, m.loss_avoider, m.loss_creator, m.AI_unblockable, 
				        		 	m.opp_unblockable, m.AI_blockable, m.opp_blockable, m.winning_column, m.three_preventer, m.bad_position, m.four_map);
		}
	}
	...
	public int max_element_array(int[] quality_arr, ArrayList<Integer> good_sim_columns, ArrayList<Integer> bad_sim_columns, ArrayList<Integer> illegal_moves,
			ArrayList<Integer> losing_moves, ArrayList<Integer> unwise_moves, ArrayList<Integer> loss_avoider, ArrayList<Integer> loss_creator, 
			ArrayList<Integer> AI_unblockable, ArrayList<Integer> opp_unblockable, ArrayList<Integer> ai_blockable, ArrayList<Integer> opp_blockable, 
