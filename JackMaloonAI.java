	...
	*/
						...
						if(getState[i][j]==0) {
							...
							for(int col=0; col<g.getNumCols(); col++) {
								for(int row=0; row<g.getNumRows(); row++) {	
									if((getState[col][row]!=0 && ((row+3<g.getNumRows() && getState[col][row]==getState[col][row+1] && getState[col][row]==getState[col][row+2] 
										&& getState[col][row]==getState[col][row+3]) || (col+3<g.getNumCols() && getState[col][row]==getState[col+1][row] 
										&& getState[col][row]==getState[col+2][row] && getState[col][row]==getState[col+3][row]) || (row+3<g.getNumRows() 
										&&  col+3<g.getNumCols() && getState[col][row]==getState[col+1][row+1] && getState[col][row]==getState[col+2][row+2] 
										&& getState[col][row]==getState[col+3][row+3]) || (row-3>=0 && col+3<g.getNumCols() && getState[col][row]==getState[col+1][row-1] 
	...
	public int columnQuality(CFGame g, moveFinder m, boolean print, ArrayList<Integer> good_sim_columns, ArrayList<Integer> bad_sim_columns) {			//plays the next move with sound logic
		...
		if(index!=-1)      //Looks to see if there is a column that can be played that guarantees win
			...
			if(print) {
				for(int num: m.loss_avoider)
				if(!bad_sim_columns.contains(num)){
					System.out.println("bad " + bad_sim_columns);
					System.out.println("los " + m.loss_avoider);
