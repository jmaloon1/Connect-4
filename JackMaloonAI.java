...
public class JackMaloonAI implements CFPlayer{
	...
	private class moveFinder{		//inner class that doesn't actually play moves, but simulates moves to see if there is a winning one or one to prevent the opponent from winning
		...
		public boolean pretendPlay(int column, boolean opp_turn) {				//simulates playing specific column. opp_turn true if opponents turn
			...
			avoidLoss(true);   //sees is a row can be won horizontally in two moves
		...
		public void avoidLoss(boolean opp_turn) {	//sees whether a row be guaranteed 4 in a row in 2 turns
			...
			for(int i=0; i<g.getNumCols()-4; i++) {
				for(int j=0;j<g.getNumRows(); j++) {
					...
					if(getState[i+2][j]!=0 && ((j==0 && getState[i][j]==0 && getState[i+1][j]==getState[i+2][j] && getState[i+3][j]==0 && getState[i+4][j]==0)
					...
					if(i>0 && i<g.getNumCols()-3 && getState[i][j]!=0  && getState[i+1][j]==0 && getState[i-1][j]==0 && getState[i+3][j]==0 && getState[i][j]==getState[i+2][j] && (j==0 
					   || (j>0 && getState[i-1][j-1]!=0 && getState[i+1][j-1]!=0 && getState[i+3][j-1]!=0 ))) {
					...
					if((i==0 || i==1) && getState[i][j]!=0 && getState[i+1][j]==0 && (getState[i+2][j]==getState[i][j] && getState[i+3][j]==0 || (getState[i+3][j]==getState[i][j] && getState[i+2][j]==0))
						&& getState[i+4][j]==0 && getState[i+5][j]==getState[i][j] && (j==0 || (getState[i+1][j-1]!=0 && (getState[i+2][j]==0 && getState[i+2][j-1]!=0 || getState[i+3][j]==0 && getState[i+3][j-1]!=0)
						&& getState[i+4][j-1]!=0)) ){
						...
						if(!opp_turn) {
							if(!loss_creator.contains(i+1))
								loss_creator.add(i+1);
							if(!loss_creator.contains(i+2) && getState[i+2][j]==0)
								loss_creator.add(i+2);
							if(!loss_creator.contains(i+3) && getState[i+3][j]==0)
								loss_creator.add(i+3);
						}
						else {
							if(!loss_avoider.contains(i+1))
